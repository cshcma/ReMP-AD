...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_layers: list = [3, 6, 9],
                attn_mask: Optional[torch.Tensor] = None,  weighted=False, mask_info=None):
        idx = 0
        out_attn = []
        # out_tokens = x
        out_tokens = []
        for r in self.resblocks:
            ...
            if self.grad_checkpointing and not torch.jit.is_scripting():
                ...
                if idx in out_layers:
                    out_tokens.append(x)
                if weighted==True  and mask_info is not None and idx in mask_info['mask_layers'] :
                        VPK_weight=self.VPKweight(x , mask_info['mask_topk'], mask_info['mask_th'], mask_info['mask_weight'])
                        x=x.permute(2,0,1)*VPK_weight
                        x=x.permute(1,2,0)

                    # out_tokens = x
        return x, out_attn, out_tokens

    def VPKweight(self,p,topk=100,mask_th=0.5,mask_weight=[1.0,1.0]):#p:L*B*dim->L*B
        p=p[1:,:,:].permute(1,0,2)
        p_norm=p/p.norm(dim=-1, keepdim=True)
        cos=p_norm @ p_norm.permute(0,2,1)
        topK_values = cos.topk(k=topk, dim=-1).values
        #topK_mean = 1 - torch.mean(topK_values, dim=-1)
        mask = 1-torch.mean(topK_values, dim=-1)
        mask[mask > mask_th], mask[mask <= mask_th] = mask_weight[0], mask_weight[1]
        mask=mask.permute(1, 0)
        _,B=mask.shape
        t=torch.ones([1,B],dtype=mask.dtype).to(mask.device)
        mask=torch.cat((t,mask),dim=0)
        return mask
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_layers: list,weighted=False,mask_info=None):
        ...
        x, attn, patch_tokens = self.transformer(x, out_layers, weighted=weighted,mask_info=mask_info)
